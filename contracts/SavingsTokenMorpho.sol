...
contract SavingsTokenMorpho is Ownable, IMorphoFlashLoanCallback {
	...
	ISavingsToken private immutable savingsToken;
	...
	constructor(
		...
		savingsToken = ISavingsToken(_collateral);
	...
	function onMorphoFlashLoan(uint256 assets, bytes calldata data) external {
		...
		if (opcode == INCREASE_LEVERAGE) {
			...
			loan.forceApprove(address(savingsToken), amountIn);
			uint256 amountOut = savingsToken.save(amountIn);
			...
			// forceApprove and execute swap
			collateral.forceApprove(address(savingsToken), amountIn);
			uint256 amountOut = savingsToken.withdraw(amountIn);
			...
			// forceApprove and execute swap
			collateral.forceApprove(address(savingsToken), p.collateral);
			uint256 amountOut = savingsToken.withdraw(p.collateral);
